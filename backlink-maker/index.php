...
function handleApi($uri, $method, $input) {
    ...
    if (preg_match('#^/api/targets/(\d+)/test$#', $uri, $m) && $method === 'POST') {
        ...
        if (!empty($res['cred']) && is_array($res['cred'])) bkPersistCred($t['id'], $res['cred']);
        unset($res['cred']);
        bkJson($res, $res['success'] ? 200 : 400);
    }
    if (preg_match('#^/api/targets/(\d+)$#', $uri, $m)) {
        ...
        if ($method === 'PUT') {
            ...
            $vals[] = $id;
            $stUp = $db->prepare('UPDATE targets SET ' . implode(', ', $sets) . ' WHERE id = ?');
            $stUp->execute($vals);
            bkJson(['success' => true]);
        }
        if ($method === 'DELETE') {
    ...
    // ---- Copy & Paste Studio (no-API sites: Hashnode etc.) ----
    if (preg_match('#^/api/targets/(\\d+)/copy-content$#', $uri, $m) && $method === 'POST') {
        $t = bkGetTargetRow(intval($m[1]));
        if (!$t) bkJson(['error' => 'Target not found.'], 404);
        $pkg = BacklinkMaker::generateCopyPackage($t);
        bkJson(['success' => true] + $pkg);
    }
    if (preg_match('#^/api/targets/(\\d+)/copy-job$#', $uri, $m) && $method === 'POST') {
        $t = bkGetTargetRow(intval($m[1]));
        if (!$t) bkJson(['error' => 'Target not found.'], 404);
        $pkg = [];
        foreach (['title', 'topic', 'angle', 'anchor', 'html', 'text', 'image_url', 'image_file'] as $k) {
            if (array_key_exists($k, $input)) $pkg[$k] = (string)$input[$k];
        }
    ...
    if (preg_match('#^/api/targets/(\\d+)/wix/draft-test$#', $uri, $m) && $method === 'POST') {
        $t = bkGetTargetRow(intval($m[1]));
        if (!$t) bkJson(['error' => 'Target not found.'], 404);
        $cred = json_decode($t['credential_json'] ?? '{}', true) ?: [];
        if (($t['platform'] ?? '') !== 'wix') bkJson(['error' => 'This site is not a Wix site.'], 400);
        $settings = getSettings();
        $main = trim((string)$settings['main_site_url']);
        $title = 'AutoBacklink draft test — ' . date('M j, Y H:i');
        $html = "<h2>Draft test from AutoBacklink</h2><p>This is a <strong>test draft</strong> created by the Site Panels → Draft Test button. It was saved as a DRAFT, so it is <em>not</em> public — open your Wix dashboard → Blog → Posts → Drafts to see it.</p><p>If you can see this, the Wix blog API works end-to-end (token, member, image, content). Our main site: " . ($main ? '<a href="' . escapeHtml($main) . '">' . escapeHtml($main) . '</a>' : '<em>(set your main site URL in Auto Runner first)</em>') . ".</p>";
        $res = BacklinkPublisher::publishWix($cred, $title, $html, '', true);
        if (!empty($res['cred']) && is_array($res['cred'])) bkPersistCred($t['id'], $res['cred']);
        bkJson(['success' => $res['success'], 'url' => $res['url'] ?? '', 'error' => $res['error'] ?? '', 'warn' => $res['warn'] ?? ''], $res['success'] ? 200 : 400);
    }

    // ---- Wix: list community groups (for the panel's group picker) ----
    if (preg_match('#^/api/targets/(\\d+)/wix/groups$#', $uri, $m)) {
        $t = bkGetTargetRow(intval($m[1]));
        if (!$t) bkJson(['error' => 'Target not found.'], 404);
        $cred = json_decode($t['credential_json'] ?? '{}', true) ?: [];
        $tk = BacklinkPublisher::wixEnsureToken($cred);
        if (!$tk['ok']) bkJson(['success' => false, 'error' => $tk['error']], 400);
        $res = BacklinkPublisher::wixListGroups($tk['cred']);
        bkPersistCred($t['id'], $res['cred'] ?? $cred);
        bkJson(['success' => $res['success'], 'groups' => $res['groups'], 'error' => $res['error']], $res['success'] ? 200 : 400);
    }

    // ---- Wix community: explore topics + prepare comments (posts NOTHING) ----
    if (preg_match('#^/api/targets/(\\d+)/community/explore$#', $uri, $m) && $method === 'POST') {
        $t = bkGetTargetRow(intval($m[1]));
        if (!$t) bkJson(['error' => 'Target not found.'], 404);
        $cred = json_decode($t['credential_json'] ?? '{}', true) ?: [];
        $tk = BacklinkPublisher::wixEnsureToken($cred);
        if (!$tk['ok']) bkJson(['success' => false, 'error' => $tk['error']], 400);
        $cred = $tk['cred'];
        $groupId = trim((string)($input['group_id'] ?? '')) ?: trim((string)($t['community_group_id'] ?? '')) ?: trim((string)($cred['group_id'] ?? ''));
        $rg = BacklinkPublisher::wixResolveGroup($cred, $groupId);
        if (!$rg['ok']) { bkPersistCred($t['id'], $rg['cred']); bkJson(['success' => false, 'error' => $rg['error']], 400); }
        $groupId = $rg['group_id'];
        if (!empty($rg['auto_picked'])) {
            $db = getDB();
            $db->prepare('UPDATE targets SET community_group_id = ? WHERE id = ?')->execute([$groupId, $t['id']]);
        }
        $list = BacklinkPublisher::wixListTopics($rg['cred'], $groupId);
        bkPersistCred($t['id'], $list['cred'] ?? $rg['cred']);
        if (empty($list['success']) || empty($list['topics'])) {
            bkJson(['success' => false, 'error' => $list['error'] ?? 'No topics.', 'tried' => array_map(fn($x) => $x['endpoint'] . ' → HTTP ' . $x['http'], $list['tried'])], 400);
        }
        $db = getDB();
        ...
        $niche = trim((string)$t['niche']);
        usort($cands, fn($a, $b) => BacklinkContent::topicRelevance($a['title'] . ' ' . $a['text'], $niche) <=> BacklinkContent::topicRelevance($b['title'] . ' ' . $b['text'], $niche));
        $settings = getSettings();
        $topics = [];
        foreach (array_slice($cands, 0, 5) as $tp) {
            $c = BacklinkContent::generateComment($settings, $t, $tp['title'], $tp['text']);
            $topics[] = [
                'id' => $tp['id'], 'title' => $tp['title'], 'text' => $tp['text'], 'url' => $tp['url'],
                'match' => round(BacklinkContent::topicRelevance($tp['title'] . ' ' . $tp['text'], $niche), 2),
                'comment' => $c['text'], 'used_ai' => $c['used_ai'],
            ];
        }
        bkJson(['success' => true, 'group_id' => $groupId, 'topics' => $topics, 'total_new' => count($cands)]);
    }

    // ---- Wix community: actually post one prepared comment ----
    if (preg_match('#^/api/targets/(\\d+)/community/post$#', $uri, $m) && $method === 'POST') {
        $t = bkGetTargetRow(intval($m[1]));
        if (!$t) bkJson(['error' => 'Target not found.'], 404);
        $cred = json_decode($t['credential_json'] ?? '{}', true) ?: [];
        $tk = BacklinkPublisher::wixEnsureToken($cred);
        ...
        if (!$rg['ok']) { bkPersistCred($t['id'], $rg['cred']); bkJson(['success' => false, 'error' => $rg['error']], 400); }
        $groupId = $rg['group_id'];
        $topicId = trim((string)($input['topic_id'] ?? ''));
        $comment = trim((string)($input['comment'] ?? ''));
        if (!$topicId || !$comment) bkJson(['error' => 'topic_id and comment are required.'], 400);
        $res = BacklinkPublisher::wixPostReply($rg['cred'], $groupId, $topicId, $comment);
        bkPersistCred($t['id'], $res['cred'] ?? $cred);
        $ok = !empty($res['success']);
        $db = getDB();
        $st = $db->prepare('INSERT INTO community_actions (target_id, group_id, topic_id, topic_url, topic_title, comment_text, status, error_message, created_at, posted_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime(\'now\'), ?)');
        $st->execute([$t['id'], $groupId, $topicId, (string)($input['topic_url'] ?? ''), (string)($input['topic_title'] ?? ''), $comment, $ok ? 'Posted' : 'Failed', $ok ? '' : ($res['error'] ?? '?'), $ok ? nowString() : null]);
        if ($ok) {
            $db->prepare("UPDATE targets SET community_last_at = datetime('now') WHERE id = ?")->execute([$t['id']]);
            addRunLog("Wix community: comment posted manually via Site Panels ({$t['name']})");
        }
        bkJson(['success' => $ok, 'error' => $ok ? '' : ($res['error'] ?? '?')], $ok ? 200 : 400);
    }

    // ---- Wix community: run the auto comment now (respect topic dedupe) ----
    if (preg_match('#^/api/targets/(\\d+)/community/run$#', $uri, $m) && $method === 'POST') {
        $t = bkGetTargetRow(intval($m[1]));
        if (!$t) bkJson(['error' => 'Target not found.'], 404);
        $res = BacklinkMaker::runCommunityOnce(getSettings(), $t);
        bkJson(['success' => $res['success'], 'topic' => $res['topic'] ?? null, 'comment' => $res['comment'] ?? '', 'error' => $res['error'] ?? ''], $res['success'] ? 200 : 400);
    }

    // ---- Run now (bypasses pacing/timers: does everything that isn't in manual mode) ----
    if ($uri === '/api/run' && $method === 'POST') {
        $summary = BacklinkMaker::runDaily(true);
...
/** Fetch one target row (or null). */
function bkGetTargetRow($id) {
    $db = getDB();
    $st = $db->prepare('SELECT * FROM targets WHERE id = ?');
    $st->execute([$id]);
    $t = $st->fetch();
...
/** Save refreshed/learned credentials back to the target. */
function bkPersistCred($targetId, array $cred) {
    $db = getDB();
