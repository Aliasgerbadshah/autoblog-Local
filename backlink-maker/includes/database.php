...
function initSchema(PDO $db) {
    ...
    // ---- Migration: per-site control panel fields (Site Panels feature) ----
    $targetCols = [];
    foreach ($db->query('PRAGMA table_info(targets)')->fetchAll() as $c) $targetCols[] = $c['name'];
    $newCols = [
        'blog_mode'              => "TEXT DEFAULT 'auto'",   // auto | scheduled | manual
        'blog_daily_count'       => 'INTEGER DEFAULT 1',      // 1..5 posts/day
        'blog_time'              => "TEXT DEFAULT ''",        // HH:MM timer (window start)
        'community_enabled'      => 'INTEGER DEFAULT 0',
        'community_daily_count'  => 'INTEGER DEFAULT 2',      // 1..3 comments/day (bot-safe)
    ...
    foreach ($newCols as $col => $def) {
        if (!in_array($col, $targetCols, true)) {
            $db->exec("ALTER TABLE targets ADD COLUMN $col $def");
        }
    }

    // ---- Community actions log (what we commented on, where, with what text) ----
    $db->exec('CREATE TABLE IF NOT EXISTS community_actions (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        target_id INTEGER NOT NULL,
