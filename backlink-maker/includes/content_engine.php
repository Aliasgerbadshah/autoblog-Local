...
class BacklinkContent {
    ...
    /**
     * How relevant is this community topic to our niche? (0..1 score)
     * Used to pick topics worth commenting on.
     */
    public static function topicRelevance($topicText, $niche) {
        $topicTokens = self::tokens((string)$topicText);
        if (!$topicTokens) return 0.0;
        $nicheTokens = self::tokens((string)$niche);
        if (!$nicheTokens) return 0.35; // no niche set → mild relevance for everything
        $overlap = count(array_intersect($nicheTokens, $topicTokens));
        $score = min(1.0, $overlap / max(1, (int)ceil(count($nicheTokens) / 2)));
        // small bonus: longer overlap beats partial
        return $score;
    }

    /**
     * Write a USEFUL community comment for a topic (not spam): 1-3 sentences
     * that actually help, ending with a natural mention of our site + link.
     */
    public static function generateComment(array $settings, array $target, $topicTitle, $topicText) {
        $mainSite = trim((string)($settings['main_site_url'] ?? ''));
        $host = $mainSite ? (parse_url($mainSite, PHP_URL_HOST) ?: $mainSite) : 'our site';
        $niche = trim((string)($target['niche'] ?? '')) ?: 'this field';

        $creds = self::getChatCreds();
        if (!empty($creds['api_key'])) {
            $prompt = "You are a helpful member of an online community ({$target['name']}). "
                . "A member posted this topic:\n\nTITLE: " . $topicTitle . "\nTEXT: " . substr((string)$topicText, 0, 700) . "\n\n"
                . "Write ONE helpful comment (max 3 short sentences) that genuinely answers or helps with the topic. "
                . "You run a $niche website at $mainSite. Mention it once, naturally, as a useful resource — like a real person would, not like an ad. "
                . "End the comment with the link on its own: $mainSite\n"
                . "Rules: no emoji, no ALL-CAPS, no 'hope this helps' filler, plain conversational English, do not quote the topic back.";
            $res = AIProviderClient::chat($creds, $prompt);
            if (!empty($res['success']) && trim((string)($res['content'] ?? ''))) {
                $text = AntiAiSanitizer::sanitizeText(trim((string)$res['content']));
                $text = preg_replace('/\s{2,}/', ' ', $text);
                if (mb_strlen($text) > 400) $text = mb_substr($text, 0, 400) . '…';
                // safety: ensure our link is present
                if (!str_contains($text, $mainSite)) $text = trim($text . "\n" . $mainSite);
                return ['text' => $text, 'used_ai' => true];
            }
        }
        // Template fallback (no chat API key)
        $snippets = [
            "Good question — from working in {$niche}, the fastest path is to keep it simple and consistent rather than over-complicating it.",
            "We ran into the same thing on our {$niche} site and solved it by starting with the basics first, then iterating.",
            "A practical answer: most people overthink this; a simple, repeatable approach works better long-term.",
        ];
        $tpl = $snippets[array_rand($snippets)];
        $text = "$tpl If you want a deeper look, we put notes and examples on our site: $mainSite";
        return ['text' => $text, 'used_ai' => false];
    }
