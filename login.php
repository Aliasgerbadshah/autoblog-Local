...
<head>
    ...
    <title>LOGIN - AutoBlog & Marketing SaaS</title>
    ...
    <style>
        :root {
            ...
            --bg-gradient: linear-gradient(135deg, #a5b4fc 0%, #c7d2fe 30%, #e0e7ff 60%, #fae8ff 100%);
            --card-bg: rgba(255, 255, 255, 0.12);
            --inner-card-bg: #ffffff;
            --primary-blue: #1b57f6;
            --primary-hover: #1243cb;
            --text-dark: #0f172a;
            --text-muted: #475569;
            --input-bg: #f8fafc;
            --border-light: #e2e8f0;
            ...
            --badge-bg: rgba(27, 87, 246, 0.1);
            --badge-color: #1b57f6;
            --badge-border: rgba(27, 87, 246, 0.2);
        }

        [data-theme="dark"] {
            /* DARK THEME VARIABLES */
            --bg-gradient: linear-gradient(135deg, #090d16 0%, #0f172a 35%, #1e1b4b 70%, #2e1065 100%);
        ...
        * {
        ...
        /* ABSTRACT ANIMATED MOVING BACKGROUND DESIGNS */
        ...
        /* Right Split Section - Inner Form Card */
        .right-section {
            flex: 0.9;
            padding: 32px;
            display: flex;
        ...
        .btn-primary {
            ...
            font-weight: 800;
            cursor: pointer;
            box-shadow: 0 6px 16px rgba(27, 87, 246, 0.28);
            text-transform: uppercase;
            letter-spacing: 0.05em;
        }
...
<body>
    ...
    <div class="outer-card">
        ...
        <div class="right-section">
            <div class="form-card">
                <span class="auth-pill-badge">🔑 MEMBER PORTAL</span>

                <div class="form-header">
                    <h2 id="form-heading">LOGIN</h2>
                    <p id="form-subheading">Welcome back! Enter your login details</p>
                </div>

                <div id="info-box" class="alert-info" style="display:none;"></div>
                <div id="error-box" class="alert-error"></div>

                <!-- STEP 1: USERNAME & PASSWORD FORM -->
                <form id="step1-form" onsubmit="handleRequestOtp(event)">
                    <div class="form-group">
                        <label>Email or Username</label>
                        <input type="text" id="username" placeholder="name@domain.com" required>
                    </div>
                    ...
                    <button type="submit" id="btn-step1" class="btn-primary">CONTINUE TO OTP ➔</button>
                </form>

                <!-- STEP 2: BREVO EMAIL OTP VERIFICATION FORM -->
                <form id="step2-otp-form" style="display:none;" onsubmit="handleVerifyOtp(event)">
                    <div class="form-group">
                        <label>Enter 6-Digit Verification Code</label>
                        <input type="text" id="otp_code" class="otp-input-field" maxlength="6" placeholder="000000" required autocomplete="off">
                    </div>

                    <button type="submit" id="btn-step2" class="btn-primary" style="margin-top:1rem;">VERIFY OTP & LOGIN 🔒</button>
                    
                    <p style="text-align:center; margin-top:12px; font-size:0.8rem; color:var(--text-muted);">
                        Didn't receive code? <a href="#" onclick="resendOtp(event)" style="color:var(--primary-blue); font-weight:700;">Resend OTP</a>
                    </p>
                </form>
    ...
    <script>
        ...
        async function handleRequestOtp(e) {
            ...
            document.getElementById('btn-step1').innerText = 'SENDING OTP...';
            ...
            if (res.ok && data.success && data.otp_required) {
                ...
                // Smooth transition to Step 2 OTP verification
                document.getElementById('form-heading').innerText = 'SECURITY OTP';
                document.getElementById('form-subheading').innerText = `Enter the code sent to ${data.masked_email}`;
                document.getElementById('step1-form').style.display = 'none';
                document.getElementById('step2-otp-form').style.display = 'block';

                let infoMsg = `🔒 A unique security code was sent via email.`;
                if (data.demo_otp_preview) {
                    infoMsg += `<br><strong>Test Mode OTP Preview:</strong> <span style="font-size:1.1rem; color:#10b981; letter-spacing:2px;">${data.demo_otp_preview}</span>`;
                }
                infoBox.innerHTML = infoMsg;
                infoBox.style.display = 'block';

            } else {
                document.getElementById('btn-step1').innerText = 'CONTINUE TO OTP ➔';
        ...
        async function handleVerifyOtp(e) {
            ...
            document.getElementById('btn-step2').innerText = 'VERIFYING...';
            ...
            if (res.ok && data.success) {
                ...
                document.getElementById('btn-step2').innerText = 'VERIFY OTP & LOGIN 🔒';
                errorBox.innerText = data.error || 'Invalid OTP verification code.';
                errorBox.style.display = 'block';
            }
        }
